...
class TestBacOdds:
    ...
    async def test_bac_odds(
        ...
        for round in range(1, MAX_ROUNDS + 1):
            ...
            # Step 3. 下注
            # 注單金額與下注玩法
            if play_types != BacPlayType.BANKER_NOCOMMISSION:
                # 如果不是免傭莊, 則使用一般取下注金額的方式
                bet_amounts = getattr(bac_bet_amounts, play_types.name.lower())
            else:
                # 如果是免傭莊, 則使用直接取下注金額的方式
                # 主要是conftest.py中, 針對免傭莊的命名不同, 是banker_nocomm, 所以沒這段if-else判斷會導致取免傭莊下注金額會出錯
                bet_amounts = bac_bet_amounts.banker_nocomm
            ...
            try:
                ...
                if game_result and settle_success:
                    ...
                    try:
                        # Expected 2. 驗證賠率計算
                        verify_result, verify_detail = await PayoutVerifier.verify_game_payout("bac", game_result, betinfos, settle_data)
                        assert verify_result is True, f"Payout verification failed: {verify_detail}"

                        # 判斷該玩法是否獲勝並更新測試狀態
                        # 修正傳入參數, 應是game_result["card_analysis"]
                        is_winning = PayoutCalculator._check_bac_winning(play_types, game_result["card_analysis"])
